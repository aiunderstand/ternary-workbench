...
| `p_ext_t` | `TMAC.T`, `TSUM.T`, `QNT.T`, `HMAX.T` + `TDOT.T` pseudo (P extension, A1.8) | horizontal sums of 0/`MAX`/mixed-sign digit vectors; `TMAC.T` mixed-sign lane products and a balanced-wrapping accumulate (`MAX + 3 = MIN + 2`); `TDOT.T` = `TMAC.T` with `X0`; `QNT.T` band inclusive at both edges and the **normative condition order** made observable with a negative threshold (`qnt(-1, -2) = +1`); `HMAX.T` positive/negative single lanes, a value in tryte lane 1, and all-negative lanes |
| `ext_regs_t` | extended register pool `e0`…`e332` = `X32`…`X364` (rebel6-abi.md) | full-range values in `e0`; pool bottom/top distinct; `eN` ≡ `X(32+N)` both directions; extended registers in ordinary ALU ops; neighbour independence; `SWAP.T` over extended registers; `X0` still hardwired zero |
